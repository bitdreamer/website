...
/* logger2CheckPW.php
   ...
   We are switching from checking the Alum table to using the Person table.
*/
   $bug = false;
...
if($em!="" && $em==$emas && $pw!="" && $pw==$pwas ) // no funny stuff
{
   ...
   $query="SELECT personID,nickName,userLevel,majmin FROM Person WHERE email='$em'"
       ." AND password='$pw'"
       ." ;";
   // echo "query=".$query."<br />";
   ...
   elseif ( mysql_num_rows( $result ) == 1 ) // one match in DB, woohoo!
   {
      ...
      $_SESSION['majmin']=$row['majmin'];
      
      if ($bug){echo"level=".$_SESSION['level']." <br /> \n";}
	  
	  $goto = $_POST['goto'];
	  if ($goto=="") { $goto = "logger6In.php"; }
       
      if (!$bug) { header("Location: ".$goto); exit; }
